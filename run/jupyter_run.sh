...
# Next, we install the dependencies of dummy.ipkg, an .ipkg file which will be read by Idris when invoked by the jupyter kernel. This file must list all necessary dependencies, including the one to linregr, otherwise we won't be able to run :module LinRegr.
